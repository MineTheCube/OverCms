...
$('form.use-ajax').submit(function(e) {
    ...
    if (FormHandler.debug.mode) console.log('Starting AJAX request..');
    ...
    $.ajax({
        ...
        success: function(json) {
            if (FormHandler.debug.mode) console.log('Get a response from server');
            ...
            try {
                ...
                if (typeof(r.location) != "undefined") {
                    if (FormHandler.debug.mode)
                        console.log('[Cancelled] Redirecting to: '+r.location);
                    else
                        setTimeout(function(){
                            if (r.location === '$')
                ...
                if (FormHandler.debug.mode) console.log('Finished request: Success');
            } catch(e) {
                if (FormHandler.debug.mode) {
                    console.log('Finished request: Error while processing ('+e+')');
                    console.log('[Cancelled] Form resending');
                } else {
        ...
        error: function(data) {
            ...
            FormHandler.state.ajax = false
            FormHandler.dom.callback(false, FormHandler.dom.form, FormHandler.dom.btn);
            if (FormHandler.debug.mode) console.log('Finished request: Server returned error');
        },
        complete: function() {
            if (typeof(FormHandler.dom.btn.data('loading-text')) != "undefined") {
            ...
            if (FormHandler.debug.mode) console.log('Request ended.');
        }
    });
    e.preventDefault();
    return false;
