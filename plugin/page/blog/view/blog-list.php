...
include ( $template . 'blog-list' . EXT);
...
if ($displaypage == 1) {
    ...
    $previous = str_replace( '%URL%', ABS_ROOT . $request['current'] . '/page/' . ($displaypage-1) . '/', $previous );
}
...
if ($displaypage == $pages) {
    ...
    $next = str_replace( '%URL%', ABS_ROOT . $request['current'] . '/page/' . ($displaypage+1) . '/', $next );
}
...
if ($pages > 5 and $displaypage >= $pages-5 and $displaypage >= 5) {
    for($i = $pages-4; $i <= $pages; $i++) {
        $url = ABS_ROOT . $request['current'] . '/page/' . ($i) . '/';
...
if ( !empty( $error ) ) {
    ...
    // List articles
    $page_member = ABS_ROOT . $page->getPage(array('type' => 'native', 'type_data' => 'member' ))['slug'] . '/';
    foreach( $articles as $article ) {
        $rows = $blog->getComments( $article['id'] )->fetchAll(PDO::FETCH_ASSOC);
        $comments = count($rows);
        $author = new User;
        $author->setup( $article['author_id'], 'id' );
        $url_article = ABS_ROOT . $request['current'] . '/' . $article['id'] . '/' . $article['slug'] . '/';
